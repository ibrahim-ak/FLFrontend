...
<template>
     <div class="flex">
          <div @mouseover="handelHover" :class="[isSideBarActive ? 'w-2/12' : 'w-12', 'bg-white', 'min-h-screen']">
               ...
               <ul>
                    <li v-for="(items, index) in SubMenulist" :key="index" class="flex flex-col gap-y-2 p-1 m-1">
                         <div class="flex items-center gap-x-2">
                              ...
                              :class="`${openSubMenuIndex ===index ? 'rotate-90' : 'rotate-0'} cursor-pointer`" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                   viewBox="0 0 24 24">
                                   <path fill="currentColor" d="m9 18l6-6l-6-6l-1.4 1.4l4.6 4.6l-4.6 4.6z" />
                              </svg>
                         </div>
                         <div v-if="openSubMenuIndex === index" >
                              <ul>
